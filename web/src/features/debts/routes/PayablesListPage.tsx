...
import { usePayables, type AgeBucket } from '../api/useDebts';
...
// Mirror of ReceivablesListPage but sums the other side of the ledger:
// what we owe suppliers. Same UX for consistency; separate page so the
// route can stay bookmarkable.
export function PayablesListPage() {
  ...
  const rows = q.data?.data.filter((p) => p.paymentStatus !== 'PAID') ?? [];
  ...
  return (
    <>
      ...
      {rows.length > 0 ? (
        <ul className="card-list" aria-label={t('debts.tab_payables')}>
          {rows.map((p) => {
            const contactName =
                ...
                <Link to={`/debts/payables/${p.id}/pay`} className="card-row">
                  <div className="card-row__header">
                    <h3 className="card-row__title">
                      {p.outstandingAmount} <span className="card-row__currency">{currCode}</span>
                    </h3>
                    <span className="badge badge--out">{t('debts.pay_action')}</span>
                  </div>
                  <div className="card-row__meta">
                    <span>{contactName}</span>
                    <span className="card-row__mono">{ageLabel(p.createdAt, t)}</span>
